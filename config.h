...
/* este arquivo consiste em várias seções
  * para criar uma combinação de trabalho, você deve pelo menos fazer suas escolhas na seção 1.
  * 1 - CONFIGURAÇÃO BÁSICA - você deve selecionar uma opção em cada bloco.
  * isso assume que você tem 4 canais conectados à sua placa com ESCs e servos padrão.
  * 2 - OPÇÕES ESPECÍFICAS DO TIPO DE COPTER - você provavelmente deseja verificar as opções para o seu tipo de helicóptero
  * 3 - CONFIGURAÇÃO DO SISTEMA RC
  * 4 - CPUs e placas alternativas - se você tiver
  * 5 - CONFIGURAÇÃO ALTERNATIVA - selecione RX alternativo (SBUS, PPM, etc.), intervalo ESC alternativo, etc. aqui
  * 6 - RECURSOS OPCIONAIS - habilitar bons recursos aqui (FlightModes, LCD, telemetria, monitor de bateria, etc.)
  * 7 - TUNING & DEVELOPER - se você sabe o que está fazendo; Você foi avisado
  * - (calibração ESCs, equilíbrio motor dinâmico / prop, diagnóstico, economia de memória ...)
  * 8 - DESCONTINUADO - esses recursos serão removidos em alguma versão futura
  */

/* Notes:
 * 1. os parâmetros marcados com (*) no comentário são armazenados no eeprom e podem ser alterados via monitor serial ou LCD.
 * 2. parâmetros marcados com (**) no comentário são armazenados no eeprom e podem ser alterados por meio da GUI
 */
...
/*************************************************************************************************/
     ...
     //---para robo equilibrio usar PITCH--------------------------------------------
    ...
    //#define REVERSE_MOTORS_DIRECTION  // inverter o sinal do eixo atual, ou seja, em vez de girar a placa do sensor
 
   ///  #define MAX_SPEED           250  // should be <= 500
  /// #define MAX_SPEED          400  // should be <= 500 // em teste com blackwii (22082020)
  ...
  #define MAX_SPEED          400  // should be <= 500
   ...
   //// onde 10 = 1 grau, deve ser <= 15 graus (ie <= 150) 
   #define MAX_TARGET_ANGLE    120  // ONDE 10 = 1 degree, should be <= 15 degree (i.e. <= 150) 
   
    #define MAX_STEERING        90   // DEVE SER <= 100
	
    #define RISE_SPEED_K		1.0f // este coeficiente significa o quão mais rápido o robô será durante a subida automática...  should be >= 0.5 but <= 2.0
    

  /****************************    Motor minthrottle    *******************************/
    /* Defina o comando de aceleração mínimo enviado para o ESC (Electronic Speed Controller)
       Este é o valor mínimo que permite que os motores funcionem em marcha lenta  */
  ...
  /****************************    Motor maxthrottle    *******************************/
    /* este é o valor máximo para os ESCs em potência total, este valor pode ser aumentado até 2000 */
    #define MAXTHROTTLE 1900

  /****************************    Mincommand          *******************************/
    /* este é o valor para os ESCs quando eles não estão armados
       em alguns casos, este valor deve ser reduzido para 900 para alguns ESCs específicos, caso contrário, eles falharam ao iniciar */
   #define MINCOMMAND  1000
 ///#define MINCOMMAND  900
  /**********************************  Velocidade I2C para configuração WMP antiga (configuração inútil para outros sensores)  *************/
    //#define I2C_SPEED 100000L     //100kHz normal mode, this value must be used for a genuine WMP
    #define I2C_SPEED 400000L   //400kHz fast mode, it works only with some WMP clones

  /***************************    Internal i2c Pullups   ********************************/
    /* enable internal I2C pull ups (in most cases it is better to use external pullups) */
    /* ATIVA  pull ups INTERNOS  */
  ...
  /**************************************************************************************/
    ...
    /***************************    Combined IMU Boards    ********************************/
      /* se você usar uma placa de sensor específica:
          envie qualquer correção a esta lista.
            Nota de Alex: Eu só tenho alguns fóruns, para outros fóruns, não tenho certeza, a informação foi coletada via fóruns rc, tenha cuidado */
   ...
   //----AQUI CONFIGURE SEM MPU 6050------------------------------------------------------------   
      #define GY_521            // Chinese 6  DOF with  MPU6050, LLC
      
  //----AQUI CONFIGURE SEM MPU 6050------A POSICAO DO SEU MPU 6050, COMO FOI INSTALADO NA PLACA
    ...
    /***************************    independent sensors    ********************************/
      /* deixe como comentado se você já marcou um quadro específico acima */
      /* I2C gyroscope */
      //#define WMP
      //#define ITG3200
      //#define MPU3050
      ...
      /* Se você tiver um quadro projetado apenas para o modo + e não puder girar o FC fisicamente para voar no modo X (ou vice-versa)
        * Você pode usar uma dessas opções para rotação de sensores virtuais em 45 graus e, em seguida, definir o tipo de multicóptero de acordo com o modo de vôo.
        * Verifique a ordem dos motores e as direções de rotação dos motores para combinar com o novo ponto frontal! Descomente apenas uma opção! */
...
/*************************************************************************************************/
  /********************************  PID Controller *********************************/
    ...
    /* NOVO: não é mais usado para coptertipos servo <== NECESSIDADE DE CORRIGIR - MOVER PARA WIKI */
   ...
   /* optionally disable stick combinations to arm/disarm the motors.
    ...
    /* se você deseja predefinir os valores mínimo / médio / máximo para servos logo após piscar, por causa de limitações físicas
      * espaço para viagens do servo, então você deve habilitar e definir todas as três opções a seguir */
  ...
  /***********************          Cam Stabilisation             ***********************/
    /* As linhas a seguir se aplicam apenas a um sistema de estabilização de inclinação / rotação. Remova o comentário da primeira ou segunda linha para ativá-la */
...
/*************************************************************************************************/
  ...
  /* nota: não há necessidade de descomentar algo nesta seção se você usar um receptor padrão*/
...
/*************************************************************************************************/
  ...
  /**************************************************************************************/
    ...
    /**************************    Hexa Motor 5 & 6 Pins    *******************************/
      /* PIN A0 and A1 instead of PIN D5 & D6 for 6 motors config and promini config
         ...
         /*
          PIN A0 e A1 em vez de PIN D5 e D6 para configuração de 6 motores e configuração de promini
          Este mod permite o uso de um receptor padrão em um mini profissional
          (não há necessidade de usar um receptor de soma PPM) 
          */
  ...
  /**************************************************************************************/
    ...
    //#define TEENSY20
   //descomente isso se você usar um teensy 2.0 com teensyduino, ele precisa rodar em 16MHz
    //#define TEENSY20
  ...
  /**************************************************************************************/
    ...
    /*********************************    pin Layout     **********************************/
      ...
      //#define A32U4ALLPINS
     //ative isso para um melhor pin layout se todos os pinos puderem ser usados => não é possível no ProMicro
    /**********************************    PWM Setup     **********************************/
      /* ative todas as 6 saídas PWM do hardware Motor 5 = D11 e 6 = D13. 
         nota: não é possível no promicro sparkfun (os pinos 11 e 13 não estão disponiveis)
         if activated:
  ...
  //só ative isso se você precisar alterar a atribuição de pinos padrão, por exemplo sua placa não possui um pino específico
  /* you may need to change PINx and PORTx plus #shift according to the desired pin! */
 
   //pode ser necessário alterar o PINx e o PORTx mudar de acordo com o pino desejado!
  #define OVERRIDE_V_BATPIN                   A0 // em vez de  A3    // Analog PIN 3

  //#define OVERRIDE_PSENSORPIN                 A1 // em vez de A2    // Analog PIN 2

  //#define OVERRIDE_LEDPIN_PINMODE             pinMode (A1, OUTPUT); // use A1 em vez de d13
  ...
  #define OVERRIDE_BUZZERPIN_PINMODE          pinMode (A2, OUTPUT); // use A2 em vez de d8
...
/*************************************************************************************************/
  ...
  /* USAR MODO DE TRANSMISSAO DO BLUETOOTH BAUD RATE 115200   */
  /* USAR MODO DE TRANSMISSAO DO BLUETOOTH BAUD RATE 115200   */
  /* USANDO BLUETOOTH BAUD RATE 115200 ---HC-05 ID-19:62:42---NAME PROK NOW  */
  ...
  /**************************************************************************************/
    ...
    /******                Gyro smoothing    **********************************/
      /* GYRO_SMOOTHING. In case you cannot reduce vibrations _and_ _after_ you have tried the low pass filter options, you
         ...
         /* GYRO_SMOOTHING. Caso você não consiga reduzir as vibrações _e_ _após_ ter tentado as opções do filtro passa-baixo, você
          pode tentar esta suavização giroscópica via cálculo da média. Não é adequado para multicópteros!
          Bons resultados para helicópteros, aviões e asas voadoras (espumas) com muitas vibrações. */
          
      //#define GYRO_SMOOTHING {20, 20, 3}    // (*) intervalos separados de média para roll, pitch, yaw
  ...
  /************************    Analog Reads              **********************************/
    ...
    /* se você quiser leituras analógicas mais rápidas, habilite isto. Isso pode resultar em resultados menos precisos, especialmente para mais de um canal analógico */
...
/*************************************************************************************************/
  ...
  /* Automatically increase throttle based on the angle of the copter
     ...
     /* Aumenta automaticamente o acelerador com base no ângulo do helicóptero
      Ideia original de Kraut Rob, primeira implementação HAdrian */

  //#define THROTTLE_ANGLE_CORRECTION 40
  
 /*************************        Advanced Headfree Mode             ********************/
 /* No modo Headfree avançado, quando o helicóptero está mais longe do que ADV_HEADFREE_RANGE metros então
     o rumo entre a posição inicial e o helicóptero se tornará a direção de controle
    SE o helicóptero chegar mais perto do que ADV_HEADFREE_RANGE metros, a direção de controle congelou para o
    rumo entre a casa e o helicóptero no ponto onde cruza a distância de metros ADV_HEADFREE_RANGE
    primeira implementação por HAdrian, mods por EOSBandi
 */

   //#define ADVANCED_HEADFREE									//O modo headfree avançado é habilitado quando não está comentado
   //#define ADV_HEADFREE_RANGE 15								//Alcance onde o modo headfree avançado está ativado
  ...
  /*******************************    Landing lights    *********************************/
     ...
     /* Luzes de pouso
      Use um pino de saída para controlar as luzes de pouso.
      Eles podem ser trocados automaticamente quando usados em conjunto
      com dados de altitude de uma unidade de sonar. */
  ...
  /********************************************************************/
    #define BUZZER
    #define RCOPTIONSBEEP         // retire o comentário se quiser que a campainha emita um bipe em qualquer opção de mudança no canal Aux1 para Aux4
    //#define ARMEDTIMEWARNING 330  // (*) Dispara um alarme após um certo tempo de estar armado [s] para salvar sua lipo (se o seu TX não tiver uma contagem regressiva)
    //#define PILOTLAMP             //Remova o comentário se estiver usando uma lâmpada piloto de aeronaves N
...
//------------ ajuste aqui para bateria lipo 3c 11.4v-------------
...
// AJUSTE EM VBATNOMINAL PARA O FATOR CONFORME A BATERIA SENDO USADA, AQUI, OU NO APLICATIVO EZ-GUI NA ABA DIGERSOS (Escala VBat).

    #define VBAT              // descomente esta linha para ativar o código vbat
    #define VBATSCALE       114 // (*) (**) mude este valor se a tensão lida da bateria for diferente da tensão real
    #define VBATNOMINAL     114 // 12,6V AJUSTE EM VBATNOMINAL PARA O FATOR CONFORME A BATERIA SENDO USADA, AQUI, OU NO APLICATIVO EZ-GUI NA ABA DIGERSOS (Escala VBat).
    #define VBATLEVEL_WARN1 107 // (*) (**) 10,7V  
    #define VBATLEVEL_WARN2  99 // (*) (**) 9.9V
    #define VBATLEVEL_CRIT   93 // (*) (**) 9.3V - condição crítica: se vbat ficar abaixo deste valor, o alarme permanente é acionado
    #define NO_VBAT          16 // Evite bipar sem bateria
///---------------------------------------------------------------------------------------------------------------------------------------



////------------ ajuste aqui para bateria lipo 4c 14.8v----COM TENSAO TOTAL 16,8 VOLTS ---------
  /* 
        *---  USAR ESTES VALORES DE RESISTORES PARA O DIVISOR DE TENSAO---
        *  para monitoramento V BAT
        após o divisor do resistor, devemos obter [0V; 5V] -> [0; 1023] no V_BATPIN analógico
...
// AJUSTE EM VBATNOMINAL PARA O FATOR CONFORME A BATERIA SENDO USADA, AQUI, OU NO APLICATIVO EZ-GUI NA ABA DIVERSOS (Escala VBat).

 ///   #define VBAT              // descomente esta linha para ativar o código vbat
 ///   #define VBATSCALE       168 // (*) (**) mude este valor se a tensão lida da bateria for diferente da tensão real
 ///   #define VBATNOMINAL    90 // AJUSTE EM VBATNOMINAL PARA O FATOR CONFORME A BATERIA SENDO USADA, AQUI, OU NO APLICATIVO EZ-GUI NA ABA DIGERSOS (Escala VBat).
 ///   #define VBATLEVEL_WARN1 107 // (*) (**) 10,7V  
///    #define VBATLEVEL_WARN2  99 // (*) (**) 9.9V
///    #define VBATLEVEL_CRIT   93 // (*) (**) 9.3V - condição crítica: se vbat ficar abaixo deste valor, o alarme permanente é acionado
///    #define NO_VBAT          16 // Evite bipar sem bateria
...
//------------ ajuste aqui para bateria lipo 6c 22.2v-------------
...
// AJUSTE EM VBATNOMINAL PARA O FATOR CONFORME A BATERIA SENDO USADA, AQUI, OU NO APLICATIVO EZ-GUI NA ABA DIGERSOS (Escala VBat).
 ...
 ///   #define VBAT              // descomente esta linha para ativar o código vbat
 ///   #define VBATSCALE       252 // (*) (**) mude este valor se a tensão lida da bateria for diferente da tensão real
///    #define VBATNOMINAL     180 // 12,6V tensão nominal da bateria completa - usado apenas para lcd.telemetria
 ///   #define VBATLEVEL_WARN1 180 // (*) (**) 10,7V  
 ///   #define VBATLEVEL_WARN2  179 // (*) (**) 9.9V
 ///   #define VBATLEVEL_CRIT   178 // (*) (**) 9.3V - condição crítica: se vbat ficar abaixo deste valor, o alarme permanente é acionado
 ///   #define NO_VBAT          16 // Evite bipar sem bateria
///---------------------------------------------------------------------------------------------------------------------------------------
...
///---------ajuste para bateria 2c 7,4v--------------------------------------------------------------------------------
 // #define VBAT              // descomente esta linha para ativar o código vbat
 //   #define VBATSCALE       74 // (*) (**) mude este valor se a tensão lida da bateria for diferente da tensão real
 //  #define VBATNOMINAL     74 // 7.4V tensão nominal da bateria completa - usado apenas para lcd.telemetria
 //  #define VBATLEVEL_WARN1 60 // (*) (**) 6.0V
 //  #define VBATLEVEL_WARN2  99 // (*) (**) 5.8V
 //  #define VBATLEVEL_CRIT   50 // (*) (**) 5.0 V - condição crítica: se vbat ficar abaixo deste valor, o alarme permanente é acionado
 //   #define NO_VBAT          16 // Evite bipar sem bateria


 /*
  * ESTE ABAIXO É ORIGINAL DO CODIGO---COMENTADO PARA TESTE DO SENSOR DA VBAT ACIMA--------------
   // #define VBAT              // descomente esta linha para ativar o código vbat
    #define VBATSCALE       131 // (*) (**) mude este valor se a tensão lida da bateria for diferente da tensão real
    #define VBATNOMINAL     126 // 12,6V tensão nominal da bateria completa - usado apenas para lcd.telemetria
    #define VBATLEVEL_WARN1 107 // (*) (**) 10,7V
    #define VBATLEVEL_WARN2  99 // (*) (**) 9.9V
    #define VBATLEVEL_CRIT   93 // (*) (**) 9.3V - condição crítica: se vbat ficar abaixo deste valor, o alarme permanente é acionado
    #define NO_VBAT          16 // Evite bipar sem bateria
*/
  ...
  /********* medidor de potência (monitoramento da capacidade da bateria)*************************/
    ...
    /* enable monitoring of the power consumption from battery (think of mAh)
      ...
      Full description and howto here http://www.multiwii.com/wiki/index.php?title=Powermeter
       ...
       /* ativar o monitoramento do consumo de energia da bateria (pense em mAh)
        permite definir o valor do alarme na GUI ou via LCD
       Descrição completa e como fazer aqui http://www.multiwii.com/wiki/index.php?title=Powermeter
        Duas opções:
        1 - difícil: - (usa sensor de hardware, após a configuração dá resultados muito bons)
        2 - macio: - (bons resultados + -5% para ESCs de pelúcia e mistério @ 2S e 3S, não são bons com SuperSimple ESC) */
  ...
  /********************************************************************/
    ...
    /* define a zona neutra do stick do acelerador durante a manutenção da altitude, a configuração padrão é
        +/- 50 não recomende e altere o valor abaixo se desejar alterá-lo. */
  ...
  /********************************************************************/
     ...
     */
     ...
     /*
      * este nome é exibido junto com o número da versão do MultiWii
      * ao ligar o LCD.
      * Se você não tem um DISPLAYD, pode habilitar LCD_TTY e
      * use o monitor serial do IDE do arduino para ver as informações.
      *
      * Você deve preservar o formato desta string!
      * Deve ter 16 caracteres no total,
      * Os últimos 4 caracteres serão substituídos pelo número da versão.
      */
...
/* os motores não giram quando o comando do acelerador está na posição baixa
        este é um método alternativo para parar imediatamente os motores */
  ...
  /********************************************************************/
    ...
    /* to log values like max loop time and others to come
       ...
       /* para registrar valores como tempo máximo de loop e outros por vir
        os valores de registro são visíveis via configuração do LCD
        definido como 1, habilite a opção 'R' para redefinir os valores, corrente máxima, altitude máxima
        definido como 2, adiciona tempos de ciclo mínimo / máximo
        definido como 3, adiciona consumo de energia adicional por motor (isso usa o grande array e é um devorador de memória, se POWERMETER <> PM_SOFT) */
...
/* apenas para desenvolvimento:
        para permitir conjuntos de configuração mais fáceis e reproduzíveis para a compilação de teste, diferentes conjuntos de parâmetros de configuração são mantidos
        juntos. Isso ajuda a detectar erros de tempo de compilação para vários recursos de uma forma coordenada.
        Não se destina a produzir seu firmware voador
        Usar:
        - não defina nenhuma opção em config.h,
        - habilite com #define COPTERTEST 1, então compile
        - se possível, verifique o tamanho
        - repetir com outros valores de 2, 3, 4 etc.
         */
